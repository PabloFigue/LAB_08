...
void enviar_numero_decimal(unsigned int numero) {
    ...
    sprintf(buffer, "%u", numero); // Formatear el número como una cadena de caracteres
    for (int i = 0; i < 6; i++) {
        TXREG = buffer[i]; // Copiar cada carácter de la cadena en el registro TXREG
        __delay_ms(4); // Esperar a que se complete la transmisión
    }
}
...
{
    int i;
    for(i = 0; i < strlen(txt); i++) {  // For del tamaño de la cadena de caracteres
        TXREG = txt[i];     //Copiar cada caracter de la cadena en el registrto TXREG
        __delay_ms(5);      // Esperar a que se complete la transmisión
    }
}
...
void setup(void)
{
   ...
   // INTCONbits.PEIE = 1; // habilitar interrupciones perifericas
    ...
    ADCON0bits.CHS = 0b0000;    //Inicialización en el Canal 0
    __delay_ms(2);
